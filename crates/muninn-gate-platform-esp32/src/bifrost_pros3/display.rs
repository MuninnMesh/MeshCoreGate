//! SSD1327 128×128 4-bit grayscale OLED driver over I2C.
//!
//! The Adafruit 4741 / 5634 family wires an SSD1327 to a SparkFun Qwiic /
//! STEMMA QT I2C connector with on-board 3V3 LDO + 12 V boost + level
//! shifters. From the host we only need a 3V3 supply and a vanilla I2C
//! transport — everything else is on-board. Default I2C address is `0x3D`
...
/// Maximum I2C payload per data write — keeps each I2C transaction short
/// enough to fit comfortably inside the SSD1327 RAM buffer and the host
/// driver's internal limits. Must be even (2 px/byte) and ≤ 32 to keep the
/// write-control byte + payload under most I2C drivers' segment limits.
const DATA_CHUNK_BYTES: usize = 16;
    ...
    I: I2c,
{
    ...
    #[allow(dead_code, reason = "exposed for callers that want runtime brightness control")]
    pub fn set_contrast(&mut self, value: u8) -> Result<(), DisplayError>
    {
        self.send_cmd(&[0x81, value])
    }

    /// Push the entire framebuffer to the panel. Chunks into small I2C data
    /// writes (each prefixed with the SSD1327 "data" control byte `0x40`).
    pub fn flush(&mut self) -> Result<(), DisplayError>
    {
        // Re-arm window to the full panel each flush so a previous partial
        // window write can't desync the cursor.
        self.send_cmd(&[0x15, 0x00, 0x3F])?;
        self.send_cmd(&[0x75, 0x00, 0x7F])?;

        let mut buf = [0u8; 1 + DATA_CHUNK_BYTES];
        buf[0] = 0x40; // data control byte
        for chunk in self.fb.chunks(DATA_CHUNK_BYTES) {
